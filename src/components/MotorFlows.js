...
export function generateModelCode({ acMotorType, acPower, acVoltage, acOption, acGearHead, acRatio }) {
  ...
  const powerPrefix = {
    '10W': '2', '15W': '3', '25W': '4', '40W': '5',
    '60W': '5', '90W': '5', '120W': '5', '140W': '6', '200W': '6'
  }[acPower];

  let prefix = `${powerPrefix}${motorTypeCode}${acPower.replace('W','')}GN-${phaseMap[acVoltage]}`;
  if (motorTypeCode === 'RK') prefix += 'M';
  if (['25W','40W'].includes(acPower) && terminalSuffix) prefix += terminalSuffix;
  if (['60W','90W','120W','140W','200W'].includes(acPower)) {
    const gu = motorTypeCode === 'IKR' ? 'RGU' : 'GU';
    prefix = `${powerPrefix}${motorTypeCode}${acPower.replace('W','')}${gu}-${phaseMap[acVoltage]}F${terminalSuffix}`;
    if (motorTypeCode === 'RK') prefix += 'M';
  }

  const headCode = acGearHead.includes('KB') ? 'GU' : 'GN';
  const suffix = `${headCode}${acRatio}${acGearHead.startsWith('SQUARE BOX') ? 'K' : acGearHead.startsWith('RIGHT') ? acGearHead.includes('SOLID') ? 'RT' : 'RC' : 'KB'}`;
...
export function renderACMotorFlow(state, setters, onConfirm) {
  ...
  const update = (setter, value) => setter(state[Object.keys(setters).find(k=>setters[k]===setter)]===value ? null : value);
    ...
    <div className="space-y-6 mt-6">
      ...
      {acMotorType && !acPower && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
              {label:'10W',img:W10Img},{label:'15W',img:W15Img},{label:'25W',img:W25Img},
              {label:'40W',img:W40Img},{label:'60W',img:W60Img},{label:'90W',img:W90Img},
              {label:'120W',img:W120Img},{label:'140W',img:W140Img},{label:'200W',img:W200Img}
            ].map(({label,img})=> (
      ...
      {acPower && !acVoltage && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[{label:'1Phase220V',img:SingleImg},{label:'3Phase220V',img:ThreeImg}].map(({label,img})=> (
      ...
      {acVoltage && !acOption && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[{label:'Fan',img:FanImg},{label:'Terminal Box',img:TmbImg},{label:'Brake',img:EmbImg},{label:'Cooling Fan',img:FcfImg},{label:'Thermal',img:TmpImg},{label:'Standard',img:StdImg}].map(({label,img})=> (
      ...
      {acOption && !acGearHead && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[{label:'K',img:GBKImg},{label:'KB',img:GBKBImg},{label:'RC',img:GBRCImg},{label:'RT',img:GBRTImg}].map(({label,img})=> (
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-4">
            {[3,3.6,5,6,7.5,9,10,12.5,15,18,20,25,30,36,40,50,60,75,90,100,120,150,180,200].map(r=> (
              <button key={r} onClick={()=>update(setAcRatio,r)} className="bg-blue-100 hover:bg-blue-300 px-3 py-1 rounded">{r}</button>
            ))}
          </div>
        </div>
      }

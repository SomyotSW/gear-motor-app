...
// Controls the <model-viewer> element via querySelector
// ─────────────────────────────────────────────────────────────────────────────
function MobileLightingControls() {
    ...
    if (autoLight) {
      lightTimer.current = setInterval(() => {
        setLightRot(r => {
          const n = (r + 2) % 360;
          const mv = getMV();
          if (mv) mv.style.setProperty('--env-rotation', n + 'deg');
          return n;
        });
      }, 30);
...
function StepViewer3D({ modelCode }) {
  ...
  return (
    <div style={S.wrap}>
      ...
      {/* ── RIGHT: Controls Panel — hidden on mobile (shown in confirm page) ── */}
  ...
  <div id="ac-summary" style={{ position:'fixed', inset:0, zIndex:500, display:'flex', flexDirection:'column', background:'#0a0c10', fontFamily:"'Sarabun',sans-serif" }}>
    ...
    <div style={{ flex:1, display:'flex', flexDirection: isMobile?'column':'row', minHeight:0, overflow: isMobile?'auto':'hidden' }}>
      ...
      <div style={{ width: isMobile?'100%':280, flexShrink:0, background:'#0f1118', borderLeft: isMobile?'none':'1px solid rgba(255,255,255,0.07)', borderTop: isMobile?'1px solid rgba(255,255,255,0.07)':'none', overflowY:'auto', display:'flex', flexDirection:'column' }}>
        ...
        {isMobile && <MobileLightingControls />}

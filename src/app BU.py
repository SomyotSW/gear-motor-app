...
RETENTION_SECONDS = 7 * 24 * 60 * 60  # 7 วัน
...
def ac_quote():
    ...
    if to_email:
        ...
        body = f"""เรียนคุณ {customer.get('name','')}

    ใบเสนอราคาของท่านถูกสร้างเรียบร้อยแล้ว
    Model: {payload.get('modelCode','')}
    Qty Motor: {qty_motor}
    Qty Gear: {qty_gear}
    ...
    # สร้าง download url (ใช้ host จาก request)
    download_url = request.host_url.rstrip("/") + f"/api/download/{saved_name}"
...
def send_email_with_pdf(to_email: str, subject: str, body: str, pdf_path: str):
    # ตั้งค่าใน ENV ตอน deploy (สำคัญ)
